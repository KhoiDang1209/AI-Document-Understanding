...
Reproduce::
    ...
    python -m docintel.scripts.eval_ragas --contracts 5 --questions 8 --seed 0
"""
...
def run(
    contracts: int, questions: int, seed: int, max_workers: int, timeout: int
) -> dict[str, Any]:
    ...
    # Index paragraph chunks of the sampled contracts into one in-memory Qdrant.
    ...
    # Run the real /ask pipeline (retrieve + generate) per question.
    samples: list[SingleTurnSample] = []
    for title, question in asked:
        response = answer_question(question, store, llm, settings, contract_id=title)
    ...
    def mean(column: str) -> float:
        return round(float(frame[column].mean()), 4)
...
def main() -> None:
    ...
    metrics = run(args.contracts, args.questions, args.seed, args.max_workers, args.timeout)
